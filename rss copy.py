...
def generate_rss(entries, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8', newline='\n') as f:
        f.write(final_xml)
def fetch_weather_data_and_update_csv():
    today = datetime.now(JST)
    yesterday = today - timedelta(days=1)
    print(f"システムクロック日付: {today.strftime('%Y/%m/%d')}")

    csv_path = 'data.csv'
    last_date_str = get_last_date_in_data_csv(csv_path)
    ...
    dlist = generate_dlist(start_date_str, yesterday_str)
    ...
    body = {
        'stationNumList': '["s47626"]',
        'aggrgPeriod': '1',
        'elementNumList': '[["201",""], ["301",""]]',
        'interAnnualType': '1',
        'ymdList': f'["{dlist[0]}", "{dlist[1]}", "{dlist[2]}", "{dlist[3]}", "{dlist[4]}", "{dlist[5]}"]',
        'optionNumList': '[]',
        'downloadFlag': 'true',
        'rmkFlag': '1',
        'disconnectFlag': '1',
    ...
    for line in csv_data:
        if line.startswith('年月日') or line.startswith('2026/') or line.strip():
            if not line.startswith('年月日'):
    ...
    new_rows = []
    for row in rows:
        if len(row) >= 7 and row[0] and row[1] and row[4] and row[5]:
            ...
            avg_wind = row[4]
            h_wind = row[5]
            k_wind = row[6] if len(row) > 6 else '1'
            new_rows.append([date_str, avg_temp, h_temp, k_temp, avg_wind, h_wind, k_wind])

    if not new_rows:
        print("No valid new weather data")
        return
    ...
    with open(csv_path, mode, encoding='utf-8', newline='\n') as f:
        writer = csv.writer(f)
        if mode == 'w':
            writer.writerow(['date', 'avg-temp', 'h', 'k', 'avg-wind', 'h', 'k'])
        for row in new_rows:
            writer.writerow(row)
def generate_dlist(start_date, end_date):
    start = datetime.strptime(start_date, '%Y/%m/%d')
    end = datetime.strptime(end_date, '%Y/%m/%d')
    dlist = []
    while start <= end:
        dlist.extend([str(start.year), str(start.year), str(start.month), str(start.month), str(start.day), str(start.day)])
        start += timedelta(days=1)
    return dlist

def get_last_date_in_data_csv(csv_path='data.csv'):
...
def fetch_weather_data_and_update_csv():
    ...
    print(f"Updated {csv_path} with new weather data")

if __name__ == '__main__':
    fetch_weather_data_and_update_csv()
